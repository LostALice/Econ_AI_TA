...
class ExcelDatabaseController:
    ...
    def insert_questions(
        self, questions: list[ValidatedQuestionModel], file_name: str, doc_type: str
    ) -> int:
        """
        插入多個題目到資料庫
        ...
        try:
            # 先刪除可能存在的舊題目，確保不會有重複
            try:
                self.logger.debug("先刪除可能存在的舊題目，確保不會有重複")
                self.database.cursor.execute(
                    "DELETE FROM excel_questions WHERE file_name = %s", (file_name,)
                )

                deleted_count = self.database.cursor.rowcount
                self.logger.info(f"Deleted row count: {deleted_count}")
            ...
            for batch_index in range(total_batches):
                ...
                # 處理批次中的每個題目
                self.logger.debug("處理批次中的每個題目")
                batch_successful = 0
                for idx, question in enumerate(batch_questions):
                    try:
                        # 從字典獲取題目資料（使用轉換後的欄位名稱）
                        self.logger.debug("從字典獲取題目資料（使用轉換後的欄位名稱）")
                        ...
                        picture = question.picture  # 可以為空

                        # 檢查圖片資料
                        self.logger.debug("檢查圖片資料")
                        ...
                        # 檢查問題內容是否超出資料庫欄位長度
                        self.logger.debug("檢查問題內容是否超出資料庫欄位長度")
                        ...
                        # self.logger.debug("檢查是否有重複題目")

                        # check_sql = """
                        # SELECT id FROM excel_questions
                        ...
                        #     self.logger.info("如果找到重複題目，使用UPDATE 語句")
                        #     update_sql = """
                        #     UPDATE excel_questions SET
                        #         question_text = %s,
                        #         option_a = %s,
                        #         option_b = %s,
                        #         option_c = %s,
                        ...
                        # else:
                        #     # 如果沒有重複，使用 INSERT 語句
                        #     self.logger.info("如果沒有重複，使用 INSERT 語句")
                        #     sql = """
                        #     INSERT INTO excel_questions
                ...
                try:
                    ...
                    self.database.connection.rollback()
                    # 回滾後，從本批次中成功計數中減去
                    self.logger.info("回滾後，從本批次中成功計數中減去")
            ...
            # 驗證實際插入數量
            self.logger.info("驗證實際插入數量")
            verify_sql = "SELECT COUNT(file_name) as count FROM excel_questions WHERE file_name = %s"
            self.database.cursor.execute(verify_sql, (file_name,))
            verify_result = self.database.cursor.fetchone()
            verify_count = int(verify_result.get("count", 0)) if verify_result else 0
    ...
    def record_file_upload(
        ...
        try:
            ...
            self.database.cursor.execute(
                """
                    INSERT INTO excel_uploaded_files (file_id, file_name, doc_type, question_count)
                    VALUES (%s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE
    ...
    def get_file_list(self, doc_type: str | None = None) -> list[dict[str, Any]]:
        ...
        try:
            if doc_type:
                ...
                sql = "SELECT file_id as fileID, file_name as fileName, doc_type as docType, upload_time as uploadTime, question_count as questionCount FROM excel_uploaded_files ORDER BY upload_time DESC"
    ...
    def get_questions_by_file(self, file_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # 先獲取實際題目數量
            count_sql = (
                "SELECT COUNT(*) as total FROM excel_questions WHERE file_name = %s"
            )
            ...
            if expected_count != actual_count:
                ...
                try:
                    update_sql = "UPDATE excel_uploaded_files SET question_count = %s WHERE file_id = %s"
    ...
    def delete_file_and_questions(self, file_id: str) -> bool:
        ...
        try:
            # 首先查詢 file_name
            sql_get_file_name = (
                "SELECT file_name FROM excel_uploaded_files WHERE file_id = %s"
            )
            ...
            # 先刪除題目
            sql_delete_questions = "DELETE FROM excel_questions WHERE file_name = %s"
            ...
            # 再刪除檔案記錄
            sql_delete_file = "DELETE FROM excel_uploaded_files WHERE file_id = %s"
    ...
    def update_questions(
        ...
        try:
            ...
            sql_get_current_ids = "SELECT id FROM excel_questions WHERE file_name = %s"
            ...
            for question in questions:
                try:
                    ...
                    if picture_data is not None:
                        update_picture_sql = (
                            "UPDATE excel_questions SET picture = %s WHERE id = %s"
                        )
                        self.database.cursor.execute(
            ...
            if deleted_ids:
                delete_sql = "DELETE FROM excel_questions WHERE id IN (%s)" % ",".join(
                    ["%s"] * len(deleted_ids)
                )
            ...
            update_file_sql = (
                "UPDATE excel_uploaded_files SET question_count = %s WHERE file_id = %s"
            )
