...
class ExcelHandler:
    ...
    def parse_excel_to_questions(file_content: str, file_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(column_mapping) < len(required_fields) - 1:  # -1 因為圖片可選
                logger.error(f"Excel 檔案欄位不符合要求，需要 {required_fields}，但找到 {list(df.columns)}")
                raise ValueError(f"Excel 檔案格式不正確，請確保包含必要欄位")
            ...
            for _, row in df.iterrows():
                ...
                for field in required_fields:
                    if field not in column_mapping.values() or pd.isna(row[field]):
                        has_null = True
                        break
                ...
                # 處理圖片欄位 (如果存在)
                if has_picture and not pd.isna(row["Picture"]):
                    # 檢查圖片是否是字符串或二進制格式
                    picture = row["Picture"]
    ...
    def save_questions_to_db(questions: List[Dict[str, Any]], file_id: str, file_name: str, doc_type: str) -> bool:
        ...
        try:
            db = DBConnection()
            success = db.insert_questions_batch(questions, file_id, file_name, doc_type)
            db.close()
            return success
        except Exception as e:
