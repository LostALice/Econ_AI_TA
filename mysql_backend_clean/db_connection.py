...
class DBConnection:
                        ...
                        try:
                            ...
                            if picture is not None:
                                if isinstance(picture, bytes):
                                    logger.info(f"第 {start_idx+idx+1} 題 - 有二進位圖片資料，大小: {len(picture)} 位元組")
                                else:
                                    logger.warning(f"第 {start_idx+idx+1} 題 - 圖片資料不是二進位格式，類型: {type(picture)}")
                                    try:
                                        # 嘗試強制轉換為二進位
                                        picture = bytes(picture)
                                        logger.info(f"第 {start_idx+idx+1} 題 - 成功將圖片轉換為二進位，大小: {len(picture)} 位元組")
                                    except Exception as e:
                                        logger.error(f"第 {start_idx+idx+1} 題 - 無法將圖片轉換為二進位，錯誤: {str(e)}")
                                        picture = None
                            else:
                                logger.info(f"第 {start_idx+idx+1} 題 - 沒有圖片資料")
                            ...
                            logger.error(f"插入第 {start_idx+idx+1} 題時發生錯誤: {insert_error}")
                            failed_inserts += 1
    ...
    def get_questions(self, file_id: str) -> Tuple[List[Dict[str, Any]], str]:
        ...
        try:
            ...
            with self.connection.cursor() as cursor:
                ...
                logger.info(f"查詢返回題目數量: {len(questions)} (檔案ID: {file_id}, 檔案名: {file_name})")
    ...
    def update_questions(self, file_id: str, questions: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        更新題目內容
            ...
            with self.connection.cursor() as cursor:
                ...
                for question in questions:
                    try:
                        ...
                        picture_data = None
                        if question.get('picture') and isinstance(question['picture'], str) and question['picture'].startswith('data:image/'):
                            try:
                                # 拆解 base64 字串
                        ...
                        if picture_data is not None:
                            update_picture_sql = "UPDATE questions SET picture = %s WHERE id = %s"
                            cursor.execute(update_picture_sql, (picture_data, question_id))
                ...
                if deleted_ids:
                    delete_sql = "DELETE FROM questions WHERE id IN (%s)" % ','.join(['%s'] * len(deleted_ids))
                    cursor.execute(delete_sql, deleted_ids)
                    result['deleted_count'] = len(deleted_ids)
                    logger.info(f"已刪除 {result['deleted_count']} 個題目：{deleted_ids}")
                
                # 更新檔案的題目數量
                new_count = len(current_ids) - len(deleted_ids)
                update_file_sql = "UPDATE uploaded_files SET question_count = %s WHERE file_id = %s"
                cursor.execute(update_file_sql, (new_count, file_id))
                
                # 提交所有更改
                self.connection.commit()
                result['success'] = True
                logger.info(f"成功更新檔案 {file_id} 的題目：更新 {result['updated_count']}，刪除 {result['deleted_count']}")
